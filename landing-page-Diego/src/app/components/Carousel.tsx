...
export const Carousel = ({ services }: ServiceGrid) => {
  ...
  return (
    <BootstrapCarousel activeIndex={index} onSelect={handleSelect} className=" w-full h-[35rem] lg:h-[40rem] bg-#DCF4FF pt-15">
      {services.map((list) => (
            ...
            {list.map((service) => (
              <Card className=' md:w-[16rem] lg:w-[20rem]'>
                ...
                <Card.Body>
                  ...
                  <Button variant="primary" className='bg-color-none'>
                    Ver detalhes
                  </Button>
                </Card.Body>
              </Card>
